...
readxl::read_xlsx(str_c(inpath, "/Table-S3_exe_go.xlsx")) %>%
  dplyr::filter(stringr::str_detect(EXE,"EX")|stringr::str_detect(go.id,"GO")) -> DB_Chemical_GO
readxl::read_xlsx(str_c(inpath, "/Table-S5_go_exd.xlsx"))  %>%
...
temp %>%
  ...
  arrange(-n) -> plt
plt$metal = factor(plt$metal, levels = plt$metal)
plt %>%
  ggplot(aes(x=log2(n)+1, y = reorder(metal,n), fill = metal)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = colorRampPalette(c("#D64358","#FFF68F"))(10)) +
  scale_x_continuous(limits = c(0,10),expand=expansion(add = c(0.2, 0.2))) +
  theme_bw() +
  theme(panel.grid = element_blank())
ggsave(str_c(outpath, "/Fig4B1-GO.png"),
       height = 12,
       width = 8,
       dpi = 600)
...
b2 %>%
  ...
  geom_bar(stat = "identity")+
  scale_fill_manual(values = colorRampPalette(c("#D6F1D0","#005A32"))(10)) +
  theme_bw()+
  theme(panel.grid = element_blank())
ggsave(str_c(outpath, "/Fig4B2-GO.png"),
       height = 12,
       width = 7,
       dpi = 600)
