...
plt %>% 
  ggplot(aes(x = dropout_loss, y = variable, fill = variable)) +
  geom_bar(stat = "summary", fun = "mean",
           width = 0.7) +
  scale_x_continuous(
                     expand = c(0.01, 0)) + 
  scale_fill_manual(values = fills[1:length(order)]) + 
...
ggsave(str_c(outpath5, "/Importance.", label, ".png"),
       height = 6,
       width = 7,
       dpi = 600)
...
plotdata2  %>% 
  ...
  geom_smooth(data = plotdata, aes(x = x, y = yhat), se = T) + 
  facet_wrap(~vname, ncol = 4, scales = "free")+
  theme_bw()+
  theme(strip.text.x = element_text(size = 8),
  axis.text.x = element_text(size = 9),
  axis.text.y = element_text(size = 9))
ggsave(str_c(outpath5, "/PD.", label, ".png"),
       height = 6,
       width = 12,
       dpi = 600)
...
# execute
#hair #1
PD.plot(df_hair.metal, hair1.list.bkmr, "Hair#1")
#hair #2
PD.plot(df_hair.metal, hair2.list.bkmr, "Hair#2")
#hair #3
PD.plot(df_hair.metal, hair3.list.bkmr, "Hair#3")
#hair #4
PD.plot(df_hair.metal, hair4.list.bkmr, "Hair#4")
#Serum #1
PD.plot(df_serum1.metal, serum1.list.bkmr, "Serum#1")
#Serum #2
PD.plot(df_serum2.metal, serum2.list.bkmr, "Serum#2")
#FF
PD.plot(df_ff.metal, ff.list.bkmr, "FF")

save.image(str_c(outpath, "/4.DALEX.Rdata"))
###########################################################################
